...
Drupal.behaviors.megamenu = { 
  ...
  if(activeParent != -1)
  {
	...
	function megamenu_display() {
      ...
      if (hoverParent.hasClass('active')) {
        hoverBin.css('margin-top', '-3px');
      };
	...
	function megamenu_sizer(){
		...
		for ( var k=0 ; k < megaBins.length ; k++ ) {
			...
			if(megaBins.eq(k).hasClass('megamenu-slots-columnar')) {
				...
				megaBins.eq(k).css( 'width' , 'auto' );
				$('#megamenu-mlid-336 .megamenu-bin').css('min-width', '189px');
				$('#megamenu-mlid-339 .megamenu-bin').css('min-width', '124px');
			}
			/* auto bin width end */
